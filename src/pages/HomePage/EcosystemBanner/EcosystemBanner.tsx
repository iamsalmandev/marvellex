import { MultiComponentCard, Section, SpacedHeading, TabsVertical } from '~/components';
import { Box, BoxProps, Container, Grid, Typography } from '@material-ui/core';
import { ashWhite, highlightGreen } from '~/styles';
...
const slidesData = [
    ...
    },
    {
        ...
        description: 'Marvellex empowers creators, brands, and companies to build a direct relationship with their audiences. Marvellex offers ultra-fast NFTs with low minting fees on the Marvellex marketplace.',
        imgUrl: marvellexMarketplace
    },
    {
        heading: 'Marvellex Blockchain',
        description: 'Marvellex blockchain will be the lynchpin of the entire ecosystem as all the products will be built on top of it.',
        imgUrl: marvellexChain
    },
    {
        heading: 'Marvellex Gold',
        description: 'Marvellex streamlines the gold supply chain and makes gold investment accessible for every investor.   It delivers secure, seamless access to the acquisition of gold, allowing investors to transact in real-time, reducing the volatility traditionally associated with the procurement of gold.',
        imgUrl: marvellexGoldImg
    },
    {
        heading: 'Marvellex Exchange',
        description: 'Marvellex will also create a cryptocurrency exchange where all the markets will be based on MLXG. Having a stake in MLXG will provide discounts in trading fees.',
...
export const EcosystemBanner: React.FC<BoxProps> = (props) => {
    ...
    return (
        <Section superTitle="The Marvellex Ecosystem" superTitleStyle={{ letterSpacing: '0.5px', padding: "0 32px" }}  {...props} style={{ background: highlightGreen, padding: "55px 0" }}  >
                    ...
                    sections={[{
                        ...
                        render: ({ index, getSectionProps }) => (
                            <div {...getSectionProps()} id="transanctions-section">
                                <EcosystemSlide key={index} slideNo={index + 1} {...slidesData[index]} />
                            </div>)
                    },
                    {
